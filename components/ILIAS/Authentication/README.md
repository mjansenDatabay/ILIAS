...
`Domain\AuthenticatedUser` is the request-scoped query port for a **fully authenticated**
ILIAS user. Anonymous sessions and missing authentication return `Result\Error`.

The port sits in `Domain/`; `SessionAuthenticatedUser` in `Infrastructure/` adapts legacy
`ilAuthSession`. When that service becomes `$use`-able in component wiring, only the
composition root changes.

## KeyValueStorage Contribution

Authentication contributes a **session** backend for
...
### ADR 0001 — Flat Session Keys for Session KeyValueStorage

**Status:** Accepted.

**Context.** `Authentication\KeyValueStorage\SessionStoragePort` implements
`ILIAS\KeyValueStorage\Port\SessionStoragePort` on top of
